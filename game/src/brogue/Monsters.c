...
creature *generateMonster(short monsterID, boolean itemPossible, boolean mutationPossible)
{
    ...
    if (mutationPossible && !(monst->info.flags & MONST_NEVER_MUTATED) && !(monst->info.abilityFlags & MA_NEVER_MUTATED) && rogue.depthLevel > gameConst->mutationsOccurAboveLevel)
    {
        ...
        if (rand_percent(mutationChance))
        {
            ...
            if (!(monst->info.flags & mutationCatalog[mutationAttempt].forbiddenFlags) && !(monst->info.abilityFlags & mutationCatalog[mutationAttempt].forbiddenAbilityFlags))
            {
...
void initializeMonster(creature *monst, boolean itemPossible)
{
    ...
    if (ITEMS_ENABLED && itemPossible && (rogue.depthLevel <= gameConst->amuletLevel) && monsterItemsHopper->nextItem && rand_percent(itemChance))
    {
    ...
    // Increment MONSTER_COUNT and construct portalName
    MONSTER_COUNT++;
...
boolean monsterHiddenBySubmersion(const creature *monst, const creature *observer)
{
    ...
    {
        if (observer && (terrainFlags(observer->loc) & T_IS_DEEP_WATER) && !observer->status[STATUS_LEVITATING])
        {
...
boolean canSeeMonster(creature *monst)
{
    ...
    if (!monsterIsHidden(monst, &player) && (playerCanSee(monst->loc.x, monst->loc.y) || monsterRevealed(monst)))
    {
        return true;
    }
    return false;
...
{
    if (cellHasTerrainFlag(defender->loc, T_OBSTRUCTS_PASSABILITY) && !(defender->info.flags & MONST_ATTACKABLE_THRU_WALLS))
    {
        return true;
    }
    if (attacker == &player)
    ...
    if ((attacker->info.flags & MONST_RESTRICTED_TO_LIQUID) && !(attacker->status[STATUS_LEVITATING]) && defender->status[STATUS_LEVITATING])
    {
        return true;
    }
...
boolean monstersAreEnemies(const creature *monst1, const creature *monst2)
{
    ...
    if (((monst1->info.flags & MONST_RESTRICTED_TO_LIQUID) && !(monst2->info.flags & MONST_IMMUNE_TO_WATER) && !(monst2->status[STATUS_LEVITATING]) && cellHasTerrainFlag(monst2->loc, T_IS_DEEP_WATER))

        || ((monst2->info.flags & MONST_RESTRICTED_TO_LIQUID) && !(monst1->info.flags & MONST_IMMUNE_TO_WATER) && !(monst1->status[STATUS_LEVITATING]) && cellHasTerrainFlag(monst1->loc, T_IS_DEEP_WATER)))
    {

        return true;
    }
    return ((monst1->creatureState == MONSTER_ALLY || monst1 == &player) != (monst2->creatureState == MONSTER_ALLY || monst2 == &player));
}
...
void resolvePronounEscapes(char *text, creature *monst)
{
    ...
    for (insert = scan = text; *scan;)
    {
        ...
        { // Didn't match any of the escape sequences; copy the character instead.
            if (*scan == '.')
            {
                capitalize = true;
            }
...
short pickHordeType(short depth, enum monsterTypes summonerType, unsigned long forbiddenFlags, unsigned long requiredFlags)
{
    ...
    {
        if (!(hordeCatalog[i].flags & forbiddenFlags) && !(~(hordeCatalog[i].flags) & requiredFlags) && ((!summonerType && hordeCatalog[i].minLevel <= depth && hordeCatalog[i].maxLevel >= depth) || (summonerType && (hordeCatalog[i].flags & HORDE_IS_SUMMONED) && hordeCatalog[i].leaderType == summonerType)))
        {
...
{
    return ((monst->info.flags & MONST_SUBMERGES) && cellHasTMFlag(monst->loc, TM_ALLOWS_SUBMERGING) && !cellHasTerrainFlag(monst->loc, T_OBSTRUCTS_PASSABILITY) && !(monst->bookkeepingFlags & (MB_SEIZING | MB_SEIZED | MB_CAPTIVE)) && ((monst->info.flags & (MONST_IMMUNE_TO_FIRE | MONST_INVULNERABLE)) || monst->status[STATUS_IMMUNE_TO_FIRE] || !cellHasTerrainFlag(monst->loc, T_LAVA_INSTA_DEATH)));
}
...
static boolean drawManacle(pos loc, enum directions dir)
{
    ...
    if (isPosInMap(newLoc) && pmapAt(newLoc)->layers[DUNGEON] == FLOOR && pmapAt(newLoc)->layers[LIQUID] == NOTHING)
    {
...
creature *spawnHorde(short hordeID, pos loc, unsigned long forbiddenFlags, unsigned long requiredFlags)
{
    ...
    if (hordeID <= 0)
    {
        ...
        do
        {
            ...
            if (isPosInMap(loc))
            {
                if (cellHasTerrainFlag(loc, T_PATHING_BLOCKER) && (!hordeCatalog[hordeID].spawnsIn || !cellHasTerrainType(loc, hordeCatalog[hordeID].spawnsIn)))
                {
    ...
    if (!isPosInMap(loc))
    {
        ...
        do
        {
            while (!randomMatchingLocation(&loc, FLOOR, NOTHING, (hordeCatalog[hordeID].spawnsIn ? hordeCatalog[hordeID].spawnsIn : -1)) || passableArcCount(loc.x, loc.y) > 1)
            {
                if (!--failsafe)
                {
                    return NULL;
...
{
    return (wpIndex >= 0 && wpIndex < rogue.wpCount && !monst->waypointAlreadyVisited[wpIndex] && rogue.wpDistance[wpIndex][monst->loc.x][monst->loc.y] >= 0 && nextStep(rogue.wpDistance[wpIndex], monst->loc.x, monst->loc.y, monst, false) != NO_DIRECTION);
}
...
static short closestWaypointIndex(creature *monst)
{
    ...
    {
        if (isValidWanderDestination(monst, i) && rogue.wpDistance[i][monst->loc.x][monst->loc.y] < closestDistance)
        {
...
boolean monsterAvoids(creature *monst, pos p)
{
    ...
    if ((p.x == rogue.downLoc.x && p.y == rogue.downLoc.y) || (p.x == rogue.upLoc.x && p.y == rogue.upLoc.y))
    {
    ...
    // dry land
    if (monst->info.flags & MONST_RESTRICTED_TO_LIQUID && !cellHasTMFlag(p, TM_ALLOWS_SUBMERGING))
    {
        return true;
    }
    ...
    {
        if (monst != &player && cellHasTMFlag(p, TM_IS_SECRET) && !(discoveredTerrainFlagsAtLoc(p) & avoidedFlagsForMonster(&(monst->info))))
        {
    ...
    {
        defender = monsterAtLoc(p);
        if (defender && !(defender->bookkeepingFlags & MB_IS_DYING) && monsterWillAttackTarget(monst, defender))
        {
    ...
    defender = monsterAtLoc(p);
    if (defender && !(defender->bookkeepingFlags & MB_IS_DYING) && monstersAreEnemies(monst, defender) && attackWouldBeFutile(monst, defender))
    {
    ...
    if (monst == &player && rogue.armor && (rogue.armor->flags & ITEM_RUNIC) && (rogue.armor->flags & ITEM_RUNIC_IDENTIFIED) && rogue.armor->enchant2 == A_RESPIRATION)
    {
    ...
    // brimstone
    if (!(monst->status[STATUS_IMMUNE_TO_FIRE]) && !(monst->info.flags & MONST_INVULNERABLE) && (tFlags & T_SPONTANEOUSLY_IGNITES) && !(cFlags & (HAS_MONSTER | HAS_PLAYER)) && !cellHasTerrainFlag(monst->loc, T_IS_FIRE | T_SPONTANEOUSLY_IGNITES) && (monst == &player || (monst->creatureState != MONSTER_TRACKING_SCENT && monst->creatureState != MONSTER_FLEEING)))
    {
        return true;
    }

    // burning wandering monsters avoid flammable terrain out of common courtesy
    if (monst != &player && monst->creatureState == MONSTER_WANDERING && (monst->info.flags & MONST_FIERY) && (tFlags & T_IS_FLAMMABLE))
    {
    ...
    if (monst != &player && monst->status[STATUS_BURNING] && (burnedTerrainFlagsAtLoc(p) & (T_CAUSES_EXPLOSIVE_DAMAGE | T_CAUSES_DAMAGE | T_AUTO_DESCENT) & ~terrainImmunities))
    {
    ...
    // fire
    if ((tFlags & T_IS_FIRE & ~terrainImmunities) && !cellHasTerrainFlag(monst->loc, T_IS_FIRE) && !(cFlags & (HAS_MONSTER | HAS_PLAYER)) && (monst != &player || rogue.mapToShore[p.x][p.y] >= player.status[STATUS_IMMUNE_TO_FIRE]))
    {
        return true;
    }

    // non-fire harmful terrain
    if ((tFlags & T_HARMFUL_TERRAIN & ~T_IS_FIRE & ~terrainImmunities) && !cellHasTerrainFlag(monst->loc, (T_HARMFUL_TERRAIN & ~T_IS_FIRE)))
    {
        return true;
    }

    // chasms or trap doors
    if ((tFlags & T_AUTO_DESCENT & ~terrainImmunities) && (!(tFlags & T_ENTANGLES) || !(monst->info.flags & MONST_IMMUNE_TO_WEBS)))
    {
        return true;
    }

    // gas or other environmental traps
    if ((tFlags & T_IS_DF_TRAP & ~terrainImmunities) && !(cFlags & PRESSURE_PLATE_DEPRESSED) && (monst == &player || monst->creatureState == MONSTER_WANDERING || (monst->creatureState == MONSTER_ALLY && !(cellHasTMFlag(p, TM_IS_SECRET)))) && !(monst->status[STATUS_ENTRANCED]) && (!(tFlags & T_ENTANGLES) || !(monst->info.flags & MONST_IMMUNE_TO_WEBS)))
    {
        return true;
    }

    // lava
    if ((tFlags & T_LAVA_INSTA_DEATH & ~terrainImmunities) && (!(tFlags & T_ENTANGLES) || !(monst->info.flags & MONST_IMMUNE_TO_WEBS)) && (monst != &player || rogue.mapToShore[p.x][p.y] >= max(player.status[STATUS_IMMUNE_TO_FIRE], player.status[STATUS_LEVITATING])))
    {
        return true;
    }

    // deep water
    if ((tFlags & T_IS_DEEP_WATER & ~terrainImmunities) && (!(tFlags & T_ENTANGLES) || !(monst->info.flags & MONST_IMMUNE_TO_WEBS)) && !cellHasTerrainFlag(monst->loc, T_IS_DEEP_WATER))
    {
        return true; // avoid only if not already in it
    }

    // poisonous lichen
    if ((tFlags & T_CAUSES_POISON & ~terrainImmunities) && !cellHasTerrainFlag(monst->loc, T_CAUSES_POISON) && (monst == &player || monst->creatureState != MONSTER_TRACKING_SCENT || monst->currentHP < 10))
    {
        return true;
    }

    // Smart monsters don't attack in corridors if they belong to a group and they can help it.
    if ((monst->info.abilityFlags & MA_AVOID_CORRIDORS) && !(monst->status[STATUS_ENRAGED] && monst->currentHP <= monst->info.maxHP / 2) && monst->creatureState == MONSTER_TRACKING_SCENT && (monst->bookkeepingFlags & (MB_FOLLOWER | MB_LEADER)) && passableArcCount(p.x, p.y) >= 2 && passableArcCount(monst->loc.x, monst->loc.y) < 2 && !cellHasTerrainFlag(monst->loc, (T_HARMFUL_TERRAIN & ~terrainImmunities)))
    {
        return true;
    }
...
static boolean moveMonsterPassivelyTowards(creature *monst, pos targetLoc, boolean willingToAttackPlayer)
{
    ...
    {
        if (distanceBetween((pos){x, y}, targetLoc) <= 1 && (dx == 0 || dy == 0))
        {                 // cardinally adjacent
...
static short awarenessDistance(creature *observer, creature *target)
{
    ...
    if ((target == &player && (pmapAt(observer->loc)->flags & IN_FIELD_OF_VIEW)) || (target != &player && openPathBetween(observer->loc, target->loc)))
    {
...
static boolean awareOfTarget(creature *observer, creature *target)
{
    ...
    else if (target == &player && !(pmapAt(observer->loc)->flags & IN_FIELD_OF_VIEW))
    {
...
void updateMonsterState(creature *monst)
{
    ...
    if (monst->creatureMode == MODE_PERM_FLEEING && (monst->creatureState == MONSTER_WANDERING || monst->creatureState == MONSTER_TRACKING_SCENT))
    {
    ...
    for (creatureIterator it = iterateCreatures(monsters); !handledPlayer || hasNextCreature(it);)
    {
        ...
        handledPlayer = true;
        if (monsterFleesFrom(monst, monst2) && distanceBetween((pos){x, y}, monst2->loc) < closestFearedEnemy && traversiblePathBetween(monst2, x, y) && openPathBetween((pos){x, y}, monst2->loc))
        {
    ...
    if ((monst->creatureState == MONSTER_WANDERING) && awareOfPlayer && (pmapAt(player.loc)->flags & IN_FIELD_OF_VIEW))
    {
    ...
    else if (monst->creatureState != MONSTER_ALLY && (monst->info.flags & MONST_FLEES_NEAR_DEATH) && monst->currentHP <= 3 * monst->info.maxHP / 4)
    {
    ...
    else if (monst->creatureMode == MODE_NORMAL && monst->creatureState == MONSTER_FLEEING && !(monst->status[STATUS_MAGICAL_FEAR]) && closestFearedEnemy >= 3)
    {

        monst->creatureState = MONSTER_TRACKING_SCENT;
    }
    else if (monst->creatureMode == MODE_PERM_FLEEING && monst->creatureState == MONSTER_FLEEING && (monst->info.abilityFlags & MA_HIT_STEAL_FLEE) && !(monst->status[STATUS_MAGICAL_FEAR]) && !(monst->carriedItem))
    {
    ...
    else if (monst->creatureMode == MODE_NORMAL && monst->creatureState == MONSTER_FLEEING && (monst->info.flags & MONST_FLEES_NEAR_DEATH) && !(monst->status[STATUS_MAGICAL_FEAR]) && monst->currentHP >= monst->info.maxHP * 3 / 4)
    {
...
void decrementMonsterStatus(creature *monst)
{
    ...
    if (monsterCanSubmergeNow(monst) && !(monst->bookkeepingFlags & MB_SUBMERGED))
    {
        ...
        {
            monst->bookkeepingFlags |= MB_SUBMERGED;
            if (!monst->status[STATUS_MAGICAL_FEAR] && monst->creatureState == MONSTER_FLEEING && (!(monst->info.flags & MONST_FLEES_NEAR_DEATH) || monst->currentHP >= monst->info.maxHP * 3 / 4))
            {
...
static void pathTowardCreature(creature *monst, creature *target)
{
    ...
    // blink to the target?
    if (distanceBetween(monst->loc, target->loc) > 10 || monstersAreEnemies(monst, target))
    {
...
{
    if ((monst->info.flags & (MONST_IMMOBILE | MONST_GETS_TURN_ON_ACTIVATION | MONST_MAINTAINS_DISTANCE)) || monst->status[STATUS_ENTRANCED] || monst->status[STATUS_CONFUSED] || monst->status[STATUS_STUCK] || monst->status[STATUS_PARALYZED] || monst->status[STATUS_MAGICAL_FEAR] || monst->status[STATUS_LIFESPAN_REMAINING] == 1 || (monst->bookkeepingFlags & (MB_SEIZED | MB_SEIZING)))
    {
...
static enum directions monsterSwarmDirection(creature *monst, creature *enemy)
{
    ...
    if (distanceBetween(monst->loc, enemy->loc) != 1 || (diagonalBlocked(monst->loc.x, monst->loc.y, enemy->loc.x, enemy->loc.y, false) || (enemy->info.flags & MONST_ATTACKABLE_THRU_WALLS)) || !monstersAreEnemies(monst, enemy))
    {
    ...
    for (int i = 0; i < 8 && targetDir == NO_DIRECTION; i++)
    {
        ...
        if (isPosInMap(newLoc) && distanceBetween(enemy->loc, newLoc) == 1 && !(pmapAt(newLoc)->flags & (HAS_PLAYER | HAS_MONSTER)) && !diagonalBlocked(monst->loc.x, monst->loc.y, newLoc.x, newLoc.y, false) && (!diagonalBlocked(enemy->loc.x, enemy->loc.y, newLoc.x, newLoc.y, false) || (enemy->info.flags & MONST_ATTACKABLE_THRU_WALLS)) && !monsterAvoids(monst, newLoc))
        {
    ...
    for (creatureIterator it = iterateCreatures(monsters); !handledPlayer || hasNextCreature(it);)
    {
        ...
        handledPlayer = true;
        if (ally != monst && ally != enemy && monstersAreTeammates(monst, ally) && monstersAreEnemies(ally, enemy) && creatureEligibleForSwarming(ally) && distanceBetween(monst->loc, ally->loc) == 1 && !diagonalBlocked(monst->loc.x, monst->loc.y, ally->loc.x, ally->loc.y, false) && !monsterAvoids(ally, monst->loc) && (distanceBetween(enemy->loc, ally->loc) > 1 || diagonalBlocked(enemy->loc.x, enemy->loc.y, ally->loc.x, ally->loc.y, false)))
        {
            ...
            for (dir = 0; dir < DIRECTION_COUNT && !alternateDirectionExists; dir++)
            {
                ...
                if (isPosInMap(newPos) && !(pmapAt(newPos)->flags & (HAS_PLAYER | HAS_MONSTER)) && distanceBetween(enemy->loc, newPos) == 1 && !diagonalBlocked(enemy->loc.x, enemy->loc.y, newPos.x, newPos.y, false) && !diagonalBlocked(ally->loc.x, ally->loc.y, newPos.x, newPos.y, false) && !monsterAvoids(ally, newPos))
                {
            ...
            if (!alternateDirectionExists)
            {
                ...
                for (creatureIterator it2 = iterateCreatures(monsters); !handledPlayer || hasNextCreature(it2);)
                {
                    ...
                    handledPlayer = true;
                    if (ally != otherEnemy && monst != otherEnemy && enemy != otherEnemy && monstersAreEnemies(ally, otherEnemy) && distanceBetween(ally->loc, otherEnemy->loc) == 1 && (!diagonalBlocked(ally->loc.x, ally->loc.y, otherEnemy->loc.x, otherEnemy->loc.y, false) || (otherEnemy->info.flags & MONST_ATTACKABLE_THRU_WALLS)))
                    {
...
boolean monsterBlinkToPreferenceMap(creature *monst, short **preferenceMap, boolean blinkUphill)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (((blinkUphill && nowPreference > bestPreference) || (!blinkUphill && nowPreference < bestPreference)) && !monsterAvoids(monst, monstNeighborLoc))
        {
    ...
    for (i = 0; i < 40; i++)
    {
        ...
        if (((blinkUphill && (nowPreference > bestPreference)) || (!blinkUphill && (nowPreference < bestPreference))) && !monsterAvoids(monst, impact))
        {
            ...
            if ((abs(impact.x - origin.x) > 1 || abs(impact.y - origin.y) > 1)
                ...
                || (cellHasTerrainFlag((pos){impact.x, origin.y}, T_OBSTRUCTS_PASSABILITY)) || (cellHasTerrainFlag((pos){origin.x, impact.y}, T_OBSTRUCTS_PASSABILITY)))
            {
                gotOne = true;
            }
            else
...
boolean monsterSummons(creature *monst, boolean alwaysUse)
{
    ...
    if (monst->info.abilityFlags & (MA_CAST_SUMMON))
    {
        ...
        if (monst->creatureState == MONSTER_ALLY)
        { // Allied summoners also count monsters on the previous and next depths.
        ...
        else if ((monst->creatureState != MONSTER_ALLY || minionCount < 5) && !rand_range(0, minionCount * minionCount * 3 + 1))
        {
...
boolean monsterIsNegatable(creature *monst)
{
    ...
    if ((monst->info.abilityFlags & ~MA_NON_NEGATABLE_ABILITIES) || (monst->bookkeepingFlags & MB_SEIZING) || (monst->info.flags & MONST_DIES_IF_NEGATED) || (monst->info.flags & NEGATABLE_TRAITS) || (monst->info.flags & MONST_IMMUNE_TO_FIRE) || ((monst->info.flags & MONST_FIERY) && (monst->status[STATUS_BURNING])) || canNegateCreatureStatusEffects(monst) || (monst->movementSpeed != monst->info.movementSpeed) || (monst->attackSpeed != monst->info.attackSpeed) || (monst->mutationIndex > -1 && mutationCatalog[monst->mutationIndex].canBeNegated))
    {
        return true;
    }
...
static boolean generallyValidBoltTarget(creature *caster, creature *target)
{
    ...
    if (caster->status[STATUS_DISCORDANT] && caster->creatureState == MONSTER_WANDERING && target == &player)
    {
    ...
    if (caster->creatureState == MONSTER_ALLY)
    {
        if (canDirectlySeeMonster(caster))
        {
            ...
            for (creatureIterator it = iterateCreatures(monsters); !handledPlayer || hasNextCreature(it);)
            {
                ...
                handledPlayer = true;
                if (monstersAreEnemies(&player, enemy) && canSeeMonster(enemy) && (pmapAt(enemy->loc)->flags & IN_FIELD_OF_VIEW))
                {
...
static boolean specificallyValidBoltTarget(creature *caster, creature *target, enum boltType theBoltType)
{
    ...
    if ((boltCatalog[theBoltType].flags & BF_TARGET_ALLIES) && (!monstersAreTeammates(caster, target) || monstersAreEnemies(caster, target)))
    {
    ...
    if (((target->info.flags & MONST_REFLECT_50) || (target->info.abilityFlags & MA_REFLECT_100)) && target->creatureState != MONSTER_ALLY && !(boltCatalog[theBoltType].flags & (BF_NEVER_REFLECTS | BF_HALTS_BEFORE_OBSTRUCTION)))
    {
    ...
    if ((boltCatalog[theBoltType].flags & BF_FIERY) && burnedTerrainFlagsAtLoc(caster->loc) & avoidedFlagsForMonster(&(caster->info)))
    {
    ...
    case BE_BECKONING:
        ...
        break;
    case BE_ATTACK:
        if (cellHasTerrainFlag(target->loc, T_OBSTRUCTS_PASSABILITY) && !(target->info.flags & MONST_ATTACKABLE_THRU_WALLS))
        {
    ...
    case BE_NEGATION:
        if (monstersAreEnemies(caster, target))
        {
            ...
            if ((target->status[STATUS_IMMUNE_TO_FIRE] || target->status[STATUS_LEVITATING]) && cellHasTerrainFlag(target->loc, (T_LAVA_INSTA_DEATH | T_IS_DEEP_WATER | T_AUTO_DESCENT)))
            {
                // Drop the target into lava or a chasm if opportunity knocks.
                return true;
            }
            if (monstersAreTeammates(caster, target) && target->status[STATUS_DISCORDANT] && !caster->status[STATUS_DISCORDANT] && !(target->info.flags & MONST_DIES_IF_NEGATED))
            {
...
static boolean isLocalScentMaximum(pos loc)
{
    ...
    {
        pos newLoc = posNeighborInDirection(loc, dir);
        if (isPosInMap(newLoc) && (scentMap[newLoc.x][newLoc.y] > baselineScent) && !cellHasTerrainFlag(newLoc, T_OBSTRUCTS_PASSABILITY) && !diagonalBlocked(loc.x, loc.y, newLoc.x, newLoc.y, false))
        {
...
static enum directions scentDirection(creature *monst)
{
    ...
    for (;;)
    {
        ...
        for (dir = 0; dir < DIRECTION_COUNT; dir++)
        {
            ...
            if (coordinatesAreInMap(newX, newY) && (scentMap[newX][newY] > bestNearbyScent) && (!(pmap[newX][newY].flags & HAS_MONSTER) || (otherMonst && canPass(monst, otherMonst))) && !cellHasTerrainFlag((pos){newX, newY}, T_OBSTRUCTS_PASSABILITY) && !diagonalBlocked(x, y, newX, newY, false) && !monsterAvoids(monst, (pos){newX, newY}))
            {
...
boolean monsterFleesFrom(creature *monst, creature *defender)
{
    ...
    if ((defender->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE)) && !(defender->info.flags & MONST_IMMOBILE))
    {
    ...
    if (monst->info.abilityFlags & MA_POISONS && defender->status[STATUS_POISONED] * defender->poisonAmount > defender->currentHP)
    {
...
static boolean allyFlees(creature *ally, creature *closestEnemy)
{
    ...
    if (distanceBetween((pos){x, y}, closestEnemy->loc) < 10 && (100 * ally->currentHP / ally->info.maxHP <= 33) && ally->info.turnsBetweenRegen > 0 && !ally->carriedMonster && ((ally->info.flags & MONST_FLEES_NEAR_DEATH) || (100 * ally->currentHP / ally->info.maxHP * 2 < 100 * player.currentHP / player.info.maxHP)))
    {
...
static void moveAlly(creature *monst)
{
    ...
    if (cellHasTerrainFlag((pos){x, y}, (T_HARMFUL_TERRAIN & ~(T_IS_FIRE | T_CAUSES_DAMAGE | T_CAUSES_PARALYSIS | T_CAUSES_CONFUSION))) || (cellHasTerrainFlag((pos){x, y}, T_IS_FIRE) && !monst->status[STATUS_IMMUNE_TO_FIRE]) || (cellHasTerrainFlag((pos){x, y}, T_CAUSES_DAMAGE | T_CAUSES_PARALYSIS | T_CAUSES_CONFUSION) && !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))))
    {
    ...
    {
        creature *target = nextCreature(&it);
        if (target != monst && (!(target->bookkeepingFlags & MB_SUBMERGED) || (monst->bookkeepingFlags & MB_SUBMERGED)) && monsterWillAttackTarget(monst, target) && distanceBetween((pos){x, y}, target->loc) < shortestDistance && traversiblePathBetween(monst, target->loc.x, target->loc.y) && (!cellHasTerrainFlag(target->loc, T_OBSTRUCTS_PASSABILITY) || (target->info.flags & MONST_ATTACKABLE_THRU_WALLS)) && (!target->status[STATUS_INVISIBLE] || rand_percent(33)))
        {
    ...
    {
        if (monsterHasBoltEffect(monst, BE_BLINKING) && ((monst->info.flags & MONST_ALWAYS_USE_ABILITY) || rand_percent(30)) && monsterBlinkToSafety(monst))
        {
        ...
        if (dir == -1 || (allySafetyMap[targetLoc.x][targetLoc.y] >= allySafetyMap[x][y]) || (!moveMonster(monst, nbDirs[dir][0], nbDirs[dir][1]) && !moveMonsterPassivelyTowards(monst, targetLoc, true)))
        {
    ...
    {
        if (closestMonster->movementSpeed < monst->movementSpeed && !(closestMonster->info.flags & (MONST_FLITS | MONST_IMMOBILE)) && closestMonster->creatureState == MONSTER_TRACKING_SCENT)
        {
    ...
    if (closestMonster && (distanceBetween((pos){x, y}, player.loc) < leashLength || (monst->bookkeepingFlags & MB_DOES_NOT_TRACK_LEADER)) && !(monst->info.flags & MONST_MAINTAINS_DISTANCE) && !attackWouldBeFutile(monst, closestMonster))
    {

        // Blink toward an enemy?
        if (monsterHasBoltEffect(monst, BE_BLINKING) && ((monst->info.flags & MONST_ALWAYS_USE_ABILITY) || rand_percent(30)))
        {
            ...
            {
                creature *target = nextCreature(&it);
                if (target != monst && (!(target->bookkeepingFlags & MB_SUBMERGED) || (monst->bookkeepingFlags & MB_SUBMERGED)) && monsterWillAttackTarget(monst, target) && distanceBetween((pos){x, y}, target->loc) < shortestDistance && traversiblePathBetween(monst, target->loc.x, target->loc.y) && (!monsterAvoids(monst, target->loc) || (target->info.flags & MONST_ATTACKABLE_THRU_WALLS)) && (!target->status[STATUS_INVISIBLE] || ((monst->info.flags & MONST_ALWAYS_USE_ABILITY) || rand_percent(33))))
                {
    ...
    else if (isPosInMap(monst->targetCorpseLoc) && !monst->status[STATUS_POISONED] && (!monst->status[STATUS_BURNING] || monst->status[STATUS_IMMUNE_TO_FIRE]))
    { // Going to start eating a corpse.
        moveMonsterPassivelyTowards(monst, monst->targetCorpseLoc, false);
        if (posEq(monst->loc, monst->targetCorpseLoc) && !(monst->bookkeepingFlags & MB_ABSORBING))
        {
            if (canSeeMonster(monst))
            {
                monsterName(monstName, monst, true);
    ...
    else if ((monst->bookkeepingFlags & MB_DOES_NOT_TRACK_LEADER) || (distanceBetween((pos){x, y}, player.loc) < 3 && (pmap[x][y].flags & IN_FIELD_OF_VIEW)))
    {
    ...
    {
        if (!(monst->bookkeepingFlags & MB_GIVEN_UP_ON_SCENT) && distanceBetween((pos){x, y}, player.loc) > 10 && monsterBlinkToPreferenceMap(monst, scentMap, true))
        {
...
void monstersTurn(creature *monst)
{
    ...
    // hunting
    if ((monst->creatureState == MONSTER_TRACKING_SCENT || (monst->creatureState == MONSTER_ALLY && monst->status[STATUS_DISCORDANT]))
        ...
        && (!(monst->info.flags & MONST_RESTRICTED_TO_LIQUID) || cellHasTMFlag(player.loc, TM_ALLOWS_SUBMERGING)))
    {
        ...
        // magic users sometimes cast spells
        if (monstUseMagic(monst) || (monsterHasBoltEffect(monst, BE_BLINKING) && ((monst->info.flags & MONST_ALWAYS_USE_ABILITY) || rand_percent(30)) && monsterBlinkToPreferenceMap(monst, scentMap, true)))
        { // if he actually cast a spell
        ...
        if (distanceBetween((pos){x, y}, player.loc) > 1 || diagonalBlocked(x, y, player.loc.x, player.loc.y, false))
        {
            for (creatureIterator it = iterateCreatures(monsters); hasNextCreature(it);)
            {
                creature *ally = nextCreature(&it);
                if (monsterWillAttackTarget(monst, ally) && distanceBetween((pos){x, y}, ally->loc) == 1 && (!ally->status[STATUS_INVISIBLE] || rand_percent(33)))
                {
        ...
        if ((monst->status[STATUS_LEVITATING] || (monst->info.flags & MONST_RESTRICTED_TO_LIQUID) || (monst->bookkeepingFlags & MB_SUBMERGED) || ((monst->info.flags & (MONST_IMMUNE_TO_WEBS | MONST_INVULNERABLE) && monsterCanShootWebs(monst)))) && pmap[x][y].flags & IN_FIELD_OF_VIEW)
        {
        ...
        if (dir == -1 || (!moveMonster(monst, nbDirs[dir][0], nbDirs[dir][1]) && !moveMonsterPassivelyTowards(monst, targetLoc, true)))
        {
            ...
            for (creatureIterator it = iterateCreatures(monsters); !handledPlayer || hasNextCreature(it);)
            {
                ...
                if (!monst->status[STATUS_MAGICAL_FEAR] // Fearful monsters will never attack.
                    && monsterWillAttackTarget(monst, ally) && distanceBetween((pos){x, y}, ally->loc) <= 1)
                {
             ...
             || ((monst->info.flags & MONST_RESTRICTED_TO_LIQUID) && !cellHasTMFlag(player.loc, TM_ALLOWS_SUBMERGING)))
    {
        ...
        // if we're standing in harmful terrain and there is a way to escape it, spend this turn escaping it.
        if (cellHasTerrainFlag((pos){x, y}, (T_HARMFUL_TERRAIN & ~T_IS_FIRE)) || (cellHasTerrainFlag((pos){x, y}, T_IS_FIRE) && !monst->status[STATUS_IMMUNE_TO_FIRE] && !(monst->info.flags & MONST_INVULNERABLE)))
        {
            if (!rogue.updatedMapToSafeTerrainThisTurn)
            {
                updateSafeTerrainMap();
        ...
        if ((monst->bookkeepingFlags & MB_FOLLOWER) && (monst->leader->bookkeepingFlags & MB_CAPTIVE) && monst->leader->currentHP > (int)(monst->info.damage.upperBound * monsterDamageAdjustmentAmount(monst) / FP_FACTOR) && monst->leader->info.turnsBetweenRegen > 0 && !(monst->info.abilityFlags & MA_POISONS) && !diagonalBlocked(monst->loc.x, monst->loc.y, monst->leader->loc.x, monst->leader->loc.y, false))
        {
...
boolean canPass(creature *mover, creature *blocker)
{
    ...
    if (blocker->status[STATUS_CONFUSED] || blocker->status[STATUS_STUCK] || blocker->status[STATUS_PARALYZED] || blocker->status[STATUS_ENTRANCED] || mover->status[STATUS_ENTRANCED])
    {
...
{
    return (!cellHasTerrainFlag(loc, T_OBSTRUCTS_PASSABILITY) || (cellHasTMFlag(loc, TM_IS_SECRET) && !(discoveredTerrainFlagsAtLoc(loc) & T_OBSTRUCTS_PASSABILITY)));
}
...
boolean knownToPlayerAsPassableOrSecretDoor(pos loc)
{
    ...
    return (!(tFlags & T_OBSTRUCTS_PASSABILITY) || ((TMFlags & TM_IS_SECRET) && !(discoveredTerrainFlagsAtLoc(loc) & T_OBSTRUCTS_PASSABILITY)));
}
...
void setMonsterLocation(creature *monst, pos newLoc)
{
    ...
    if (playerCanSee(newLoc.x, newLoc.y) && cellHasTMFlag(newLoc, TM_IS_SECRET) && cellHasTerrainFlag(newLoc, T_OBSTRUCTS_PASSABILITY))
    {
...
boolean moveMonster(creature *monst, short dx, short dy)
{
    ...
    // Caught in spiderweb?
    if (monst->status[STATUS_STUCK] && !(pmap[newX][newY].flags & (HAS_PLAYER | HAS_MONSTER)) && cellHasTerrainFlag((pos){x, y}, T_ENTANGLES) && !(monst->info.flags & MONST_IMMUNE_TO_WEBS))
    {
    ...
    else
    {
        if (monst->bookkeepingFlags & MB_SEIZED)
        {
            for (creatureIterator it = iterateCreatures(monsters); hasNextCreature(it);)
            {
                ...
                if ((defender->bookkeepingFlags & MB_SEIZING) && monstersAreEnemies(monst, defender) && distanceBetween(monst->loc, defender->loc) == 1 && !diagonalBlocked(monst->loc.x, monst->loc.y, defender->loc.x, defender->loc.y, false))
                {
    ...
    if (handleWhipAttacks(monst, dir, NULL) || handleSpearAttacks(monst, dir, NULL))
    {
    ...
    if (((defender && (defender->info.flags & MONST_ATTACKABLE_THRU_WALLS)) || (isPassableOrSecretDoor((pos){newX, newY}) && !diagonalBlocked(x, y, newX, newY, false) && isPassableOrSecretDoor((pos){x, y}))) && (!defender || canPass(monst, defender) || monsterWillAttackTarget(monst, defender)))
    {
        ...
        if (defender)
        {
            if (canPass(monst, defender))
            {
                ...
                { // don't want a flying monster to swap a non-flying monster into lava!
            ...
            else
            {
                ...
                for (i = 0; i < 16; i++)
                {
                    if (hitList[i] && monsterWillAttackTarget(monst, hitList[i]) && !(hitList[i]->bookkeepingFlags & MB_IS_DYING) && !rogue.gameHasEnded)
                    {
    ...
    for (maxPermissibleDifference = 1; maxPermissibleDifference < max(DCOLS, DROWS); maxPermissibleDifference++)
    {
        for (i = 0; i < DCOLS; i++)
        {
            for (j = 0; j < DROWS; j++)
            {
                ...
                if (dist <= maxPermissibleDifference && dist > 0 && !(pmap[sCols[i]][sRows[j]].flags & (HAS_PLAYER | HAS_MONSTER)) && !monsterAvoids(monst, (pos){sCols[i], sRows[j]}) && !(monst == &player && cellHasTerrainFlag((pos){sCols[i], sRows[j]}, T_PATHING_BLOCKER)))
                {
                             ...
                             boolean deterministic)
{
    ...
    for (int k = 0; k < max(DROWS, DCOLS) && !candidateLocs; k++)
    {
        for (int i = target.x - k; i <= target.x + k; i++)
        {
            ...
            {
                if (coordinatesAreInMap(i, j) && (i == target.x - k || i == target.x + k || j == target.y - k || j == target.y + k) && (!blockingMap || !blockingMap[i][j]) && !cellHasTerrainFlag((pos){i, j}, forbiddenTerrainFlags) && !(pmap[i][j].flags & forbiddenMapFlags) && (!forbidLiquid || pmap[i][j].layers[LIQUID] == NOTHING) && (hallwaysAllowed || passableArcCount(i, j) < 2))
                {
                    candidateLocs++;
                }
            }
                                 ...
                                 boolean deterministic)
{
    ...
    for (int k = 0; k < max(DROWS, DCOLS) && !candidateLocs; k++)
    {
        for (int i = target.x - k; i <= target.x + k; i++)
        {
            for (int j = target.y - k; j <= target.y + k; j++)
            {
                if (coordinatesAreInMap(i, j) && (i == target.x - k || i == target.x + k || j == target.y - k || j == target.y + k) && grid[i][j])
                {
...
void toggleMonsterDormancy(creature *monst)
{
    ...
    if (removeCreature(dormantMonsters, monst))
    {
        ...
        if (pmapAt(monst->loc)->flags & (HAS_MONSTER | HAS_PLAYER))
        { // Occupied!
...
static void getMonsterAbilitiesText(const creature *monst, char *abilitiesText, boolean includeNegatable, boolean includeNonNegatable)
{
    ...
    if ((includeNegatable && monst->attackSpeed != monst->info.attackSpeed) || (includeNonNegatable && monst->attackSpeed == monst->info.attackSpeed))
    {
    ...
    if ((includeNegatable && monst->movementSpeed != monst->info.movementSpeed) || (includeNonNegatable && monst->movementSpeed == monst->info.movementSpeed))
    {
    ...
    for (int i = 0; monst->info.bolts[i] != BOLT_NONE; i++)
    {
        ...
        {
            if ((includeNegatable && !(boltCatalog[monst->info.bolts[i]].flags & BF_NOT_NEGATABLE)) || (includeNonNegatable && (boltCatalog[monst->info.bolts[i]].flags & BF_NOT_NEGATABLE)))
            {
    ...
    for (int i = 0; i < 32; i++)
    {
        if ((monst->info.abilityFlags & (Fl(i))) && monsterAbilityCatalog[i].description[0])
        {
            if ((includeNegatable && monsterAbilityCatalog[i].isNegatable) || (includeNonNegatable && !monsterAbilityCatalog[i].isNegatable))
            {
    ...
    for (int i = 0; i < 32; i++)
    {
        if ((monst->info.flags & (Fl(i))) && monsterBehaviorCatalog[i].description[0])
        {
            if ((includeNegatable && monsterBehaviorCatalog[i].isNegatable) || (includeNonNegatable && !monsterBehaviorCatalog[i].isNegatable))
            {
    ...
    for (int i = 0; i < 32; i++)
    {
        ...
        {
            if ((includeNegatable && (monst->bookkeepingFlags & MB_SEIZING)) || (includeNonNegatable && !(monst->bookkeepingFlags & MB_SEIZING)))
            {
...
static boolean staffOrWandEffectOnMonsterDescription(char *newText, item *theItem, creature *monst)
{
    ...
    if ((theItem->category & (STAFF | WAND)) && tableForItemCategory(theItem->category)[theItem->kind].identified)
    {
...
void monsterDetails(char buf[], creature *monst)
{
    ...
    if (!(monst->info.flags & MONST_ATTACKABLE_THRU_WALLS) && cellHasTerrainFlag(monst->loc, T_OBSTRUCTS_PASSABILITY))
    {
    ...
    else if (rogue.armor && (rogue.armor->flags & ITEM_RUNIC) && (rogue.armor->flags & ITEM_RUNIC_IDENTIFIED) && rogue.armor->enchant2 == A_IMMUNITY && monsterIsInClass(monst, rogue.armor->vorpalEnemy))
    {
    ...
    else
    {
        ...
        {
            combatMath = ((player.currentHP + (monst->info.damage.upperBound * monsterDamageAdjustmentAmount(monst) / FP_FACTOR) - 1) * FP_FACTOR) / (monst->info.damage.upperBound * monsterDamageAdjustmentAmount(monst));
            if (combatMath < 1)
            {
                combatMath = 1;
            }
    ...
    else if (rogue.weapon && (rogue.weapon->flags & ITEM_RUNIC) && (rogue.weapon->flags & ITEM_RUNIC_IDENTIFIED) && rogue.weapon->enchant2 == W_SLAYING && monsterIsInClass(monst, rogue.weapon->vorpalEnemy))
    {
    ...
    if (printStaffOrWandEffect)
    {
        ...
        {
            if ((theItem->category == STAFF || theItem->category == WAND) && tableForItemCategory(theItem->category)[theItem->kind].identified && staffOrWandEffectOnMonsterDescription(newText, theItem, monst))
            {
