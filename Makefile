...
cflags := -Isrc/brogue -Isrc/platform -Isrc/variants -Isrc/metrics -std=c99 \
...
sources := $(wildcard src/brogue/*.c) $(wildcard src/variants/*.c) $(addprefix src/platform/,main.c platformdependent.c null-platform.c) src/metrics/metrics_server.c
objects :=
